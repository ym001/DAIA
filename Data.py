...
class Data:  
	...
	def __init__(self,p): # constructeur
		self.jeux=p.jeux
		self.pourcentage_de_la_base=p.pourcentage_de_la_base
		self.taille_par_nb_echantillon=p.taille_par_nb_echantillon
		self.nb_echantillon=p.nb_echantillon
		self.path_fichier='imdb'
		if p.jeux=="imdb":
			self.path=self.path_fichier+'imdb.json'
	...
	def lecture_du_jeu_de_imdbkeras():
		...
		print('x train 0 : '+str(x_train_imdb[0]))
		print('y train 0 : '+str(y_train_imdb[0]))
		
		return x_train_imdb,y_train_imdb
		
	def lecture(self): # lecture des données -> dataframe df
		self.df=pd.read_json(self.path)
		self.df_doc=self.df[0]
		self.df_label=self.df[1]
		
	def reduction_train(self):
		print("taille des doc "+str(len(self.df_doc)))
		if self.taille_par_nb_echantillon==True:
			if self.nb_echantillon<self.df_doc.shape[0]:
				proportion=1-self.nb_echantillon*1.0/self.df_doc.shape[0]
				self.df_train_doc, self.df_test_doc, self.df_train_label, self.df_test_label = train_test_split(self.df_doc, self.df_label, test_size=proportion, random_state=42,stratify=self.df_label)
